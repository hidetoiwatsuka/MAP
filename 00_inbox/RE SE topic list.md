...
| ---- | ------------------- | -------------------------------------------------------------------- | ------------------ |
| 1    | [環境構築 + 最初のプログラム](環境構築%20+%20最初のプログラム.md) | gcc をセットアップ。`printf` で文字列と数値を表示。`int`, `float`, `char` の違い           | ここが全ての出発点          |
| 2    | [変数と制御構文](変数と制御構文.md)         | `if`/`else`, `for`, `while`, `switch`。簡単な計算プログラム（摂氏⇔華氏変換、FizzBuzzなど） | 分岐と繰り返し = プログラムの骨格 |
